...
import { drawCircleOuterTangents } from "./lumic/geomerty.js";

const colorTheme = tealTheme;
const rMandala = 180;
const colRepeat = 1;
...
function drawCircleConnectors2(circles) {
  circles.forEach((circ) => {
    ...
    if (other) {
      stroke(255);
      // stroke(getColor(colorTheme, floor(other.colIndex / colRepeat)));
      fill(getColor(colorTheme, floor(circ.colIndex / colRepeat)));
      // fill(getColor(colorTheme, floor(circ.colIndex / colRepeat)));
      strokeWeight(avg(circ.radius, other.radius) * 0.15);
      drawCircleOuterTangents(circ, other, true);
    }
  });
}
