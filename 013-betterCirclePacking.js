import { distance2, getRandom, line2D, sizes, TAU, vec2 } from './lumic/common.js'
  ...
  while (i < params.maxIterations) {
      ...
      maxD < params.sdfThreshold)
    {
      ...
      for (const circ of params.startCircles) {
        ...
        const d = dMultiplier * (distance2(p, circ.center) - circ.radius) - margin
...
function drawCircles(circles) {
  circles.forEach((circ) => {
    ...
    if (other) {
      const dir = p5.Vector.normalize(vec2(other.center.x - circ.center.x, other.center.y - circ.center.y))

      const a = p5.Vector.add(circ.center, p5.Vector.mult(dir, circ.radius - margin))
      const b = p5.Vector.add(circ.center, p5.Vector.mult(dir, margin))
      
      const c = p5.Vector.add(other.center, p5.Vector.mult(dir, -other.radius + margin))
