...
import { getRandomColor } from "../lumic/palettes.js";
import { sdCircle, sdHeart } from "../lumic/sdf.js";

const colors = ["#abbaab", "#bbaabb", "#aabbba", "#baabba", "#baabab"];
...
function pack() {
  const packingParams = {
    ...
    sdfMarginFunc: (d) => (sin((TAU * (d + 100)) / 75) + 1) * 10,
    pageMargin: 10,
  };
...
function drawCircles(circles) {
  //   noFill();
  //   stroke(colors[0]);
  //   circle(0, 0, 0.99 * height);

  noStroke();

  circles.forEach((circ) => {
    let col = color(getRandom(colors));

    let d =
      (distance2d(circ.center, vec2(0, 0)) + circ.radius) / (0.5 * height);
