...
AFRAME.registerComponent("hexgrid", {
  ...
  updateHexyMesh: function () {
    ...
    for (let i = 0; i < this.shapes.length; i++) {
      ...
      if (!isAnimableTile(shape.cOddr.x, shape.cOddr.y, this.data.xlim, this.data.ylim)) {
        animTurns = 0;
      }
  ...
  tick: function (time, timeDelta) {
    ...
    if (!this.attribInit) {
      ...
      this.mesh.geometry.setAttribute("uv", new THREE.BufferAttribute(this.uvs, 2));
      this.mesh.geometry.setAttribute("pivot", new THREE.BufferAttribute(this.pivots, 2));
      this.mesh.geometry.setAttribute("turns", new THREE.BufferAttribute(this.turns, 1));
      this.mesh.geometry.setAttribute("position", new THREE.BufferAttribute(this.vertices, 3));
